...
清缓存恢复流程：
用户打开小程序 -> `login` -> `recoverData` 分页拉取用户习惯、策略和全部 `daily_checkin_states` -> 客户端校验完整快照 -> `storageService` 暂存并提交缓存 -> `migrationService` 补字段 -> `reportService` 重新计算 -> 页面恢复。
V1 清缓存和换设备恢复全部云端每日状态。客户端必须完成全部分页、协议版本、快照令牌、数量、唯一键和关联关系校验后才替换本地缓存；中断或失败时不得用半份结果覆盖已有数据。云端无数据则初始化空用户。用户主动清缓存前如存在未同步操作，必须阻止清理并保留本地数据。
