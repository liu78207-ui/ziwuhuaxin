...
- 云函数单次响应以 5MB 为业务安全上限，为平台 6MB 同步响应限制预留开销；超过上限必须安全失败，不得覆盖本地缓存。客户端仍应通过分页恢复全部历史，不以 5MB 限制累计快照。
- 完整快照必须拒绝重复 `userHabitId`、重复 `policyVersionId` 和重复 `userHabitId + date` 每日状态。
...
- 记录 `syncLogs`。
- 提示建议：“数据恢复失败，可稍后重试”。
- 提交核心缓存失败时必须回滚旧缓存；回滚也失败时保留恢复事务和暂存数据，下一次启动继续完成提交。
